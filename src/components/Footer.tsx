      ...
      <Container>
        <div className="mb-24 grid grid-cols-1 gap-16 md:grid-cols-2 lg:grid-cols-4">
          <div className="space-y-6">
            <Link to="/" className="inline-flex items-center" aria-label={settings.siteName || 'МАМОНТОВ - дизайн и ремонт'}>
              <img
                src={logoWhite}
                alt={settings.siteName || 'МАМОНТОВ - дизайн и ремонт'}
                className="h-[48px] w-auto shrink-0 object-contain"
              />
            </Link>
          ...
          <div className="space-y-6">
            <h4 className="text-xs font-semibold uppercase tracking-widest text-brand-support">Навигация</h4>
          ...
          <div className="space-y-6">
            <h4 className="text-xs font-semibold uppercase tracking-widest text-brand-support">Сценарии</h4>
            <ul className="space-y-4 text-sm font-light text-brand-primary/80">
          ...
          <div className="space-y-6">
            <h4 className="text-xs font-semibold uppercase tracking-widest text-brand-support">Контакты</h4>
            <ul className="space-y-4 text-sm font-light text-brand-primary/80">
              <li className="flex items-center gap-3">
                <Phone className="h-4 w-4 text-brand-support" />
                <a href={`tel:${settings.phone.replace(/[^\d+]/g, '')}`} className="transition-colors hover:text-brand-accent">{settings.phone}</a>
              </li>
              <li className="flex items-center gap-3">
                <Send className="h-4 w-4 text-brand-support" />
        ...
        <div className="flex flex-col items-center justify-between gap-4 border-t border-brand-primary/10 pt-8 text-xs uppercase tracking-widest text-brand-primary/40 md:flex-row">
          <p>{currentYear} {settings.siteName || 'МАМОНТОВ - дизайн и ремонт'}. ALL RIGHTS RESERVED.</p>
